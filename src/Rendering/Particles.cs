...
public sealed class Particles
{
    ...
    public void Update(float dt, Planet planet, Cells? cells = null)
    {
        ...
        for (var i = _list.Count - 1; i >= 0; i--)
        {
            ...
            if (p.JetScale > 1f && p.Fluid == (byte)Material.Fire)
            {
                DbgJetTotal++;
                if (p.Hidden) DbgJetHidden++;
                else SmokeJetLive = true;
            }
            ...
            if (p.CollideTiles && planet.IsSolidAt(next))
            {
                ...
                // Rain (the only colliding backdrop grain) barely ricochets: 0.4 bounced
                // ~50% too high per user, 0.33 was STILL too high — 0.15 leaves a ~1-2px
                // hop that reads as the drop dying in its splash, not bouncing.
                p.Velocity = (p.Velocity - n * dot * 1.5f) * (p.Backdrop ? 0.15f : 0.4f);
                ...
                if (p.Velocity.LengthSquared() < 4f)
                {
                    ...
                    if (p.LandMat != 0 && cells != null)
                    {
                        ...
                        // BIG fluid blobs (JetScale carriers — the volcano's spew and
                        // fountain globs) deposit their VOLUME: extra cells jittered
                     ...
                     && (p.Backdrop || !cells.LiquidAtWorld(p.Position)))
            {
                ...
                // from outside; grains born inside one, like mining chips and rising
                // bubbles, are exempt): colliding effects interact with pools instead of
                // sailing through the body.
                // Backdrop grains (rain) skip the crossing test and die WHENEVER they're
                // found in liquid: a pool's top cell layer churns, so a drop could thread
                // a momentarily-open surface cell — after which both samples read liquid,
                ...
                // The drop fades out AT the contact point — resting it a step short left
                // a grain hanging above the waterline; letting it run the whole step sank
                // it. Re-entering this branch next frame is harmless: velocity is zero,
                // so no crown, and the payload stamp already cleared.
                next = contact;
            }
            p.Position = next;
            _list[i] = p;
