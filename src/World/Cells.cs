...
public sealed class Cells
{
    ...
    private void TickLiquid(int cx, int cy, float dt)
    {
        ...
        if (onLiquid)
        {
            ...
            var bSrc = _srcTile[bi];
            onRainPool = belowMat == Material.Water && (bSrc == RainWaterSrc || bSrc == DripWaterSrc);
        }
        ...
        bool skipLiquids = false, bool skipLava = false)
    {
        ...
        if (stride == 1)
        {
            ...
            foreach (var f in _flying)
            {
                ...
                // A flying cell INSIDE a liquid body is under its surface — the pool hides
                // it (the volcano's bore-jet slugs used to streak visibly up through the
                // crater lava). It reappears the frame it breaks out.
                if (LiquidAtWorld(f.Pos)) continue;
